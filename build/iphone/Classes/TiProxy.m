...
//Rare exceptions to indicate a bug in the ________ code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *________ = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:________]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org._______.module"
