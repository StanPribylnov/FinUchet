...
function Controller() {
    ...
    function submitData() {
        if ("" != ($.amount.value && cur && $.psRait.value && acc && $.comment.value)) if (0 == parseFloat($.amount.value) || 0 == parseFloat($.psRait.value)) alert("Поля сумма и курс не должны быть нулевыми!"); else {
        ...
        } else alert("Вы должны заполнить все поля");
    }
    ...
    $.__views.historyLayout.add($.__views.__alloyId2);
    submitData ? $.addListener($.__views.__alloyId2, "click", submitData) : __defers["$.__views.__alloyId2!click!submitData"] = true;
    ...
    __defers["$.__views.__alloyId2!click!submitData"] && $.addListener($.__views.__alloyId2, "click", submitData);
    _.extend($, exports);
}
